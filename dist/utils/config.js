...
class Config {
    ...
    static loadFromInputs() {
        try {
            ...
            throw new types_1.ConfigError(`Failed to load configuration: ${error instanceof Error ? error.message : 'Unknown error'}`);
        }
    }
    static getProvider() {
        ...
        if (!validProviders.includes(provider)) {
            throw new types_1.ConfigError(`Invalid provider: ${provider}. Valid options: ${validProviders.join(', ')}`);
        }
        return provider;
    }
    static getApiKey() {
    ...
    static getReviewLevel() {
        ...
        if (!validLevels.includes(reviewLevel)) {
            throw new types_1.ConfigError(`Invalid review_level: ${reviewLevel}. Valid options: ${validLevels.join(', ')}`);
        }
        return reviewLevel;
    }
    ...
    static getMaxChunkLines() {
        ...
        if (isNaN(maxLines) || maxLines < 50 || maxLines > 2000) {
            throw new types_1.ConfigError(`Invalid max_chunk_lines: ${input}. Must be a number between 50 and 2000`);
        }
        return maxLines;
    }
    static getRules() {
    ...
    static getMaxRetries() {
        ...
        if (isNaN(maxRetries) || maxRetries < 0 || maxRetries > 10) {
            throw new types_1.ConfigError(`Invalid max_retries: ${input}. Must be a number between 0 and 10`);
        }
        return maxRetries;
    }
    static getRetryDelay() {
        ...
        if (isNaN(retryDelay) || retryDelay < 100 || retryDelay > 60000) {
            throw new types_1.ConfigError(`Invalid retry_delay: ${input}. Must be a number between 100 and 60000 (milliseconds)`);
        }
        return retryDelay;
    }
    static validateConfig(config) {
        ...
        const hasConflict = config.includeGlobs.some(include => config.excludeGlobs.some(exclude => include === exclude));
        if (hasConflict) {
    ...
    static getPullRequestNumber() {
        const prNumber = process.env.GITHUB_EVENT_NAME === 'pull_request'
            ? process.env.GITHUB_EVENT_PATH
                ? require(process.env.GITHUB_EVENT_PATH).number
                : undefined
            : core.getInput('pr_number', { required: false });
        if (!prNumber) {
            throw new types_1.ConfigError('Pull request number not found. This action should run on pull_request events.');
        }

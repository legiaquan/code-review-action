...
class FileUtils {
     ...
     */
    static filterFiles(files, includeGlobs, excludeGlobs) {
        return files.filter(file => {
            ...
            const isIncluded = includeGlobs.length === 0 ||
                includeGlobs.some(pattern => (0, minimatch_1.minimatch)(file.filename, pattern));
            if (!isIncluded) {
                return false;
            }
            // Check exclude patterns
            const isExcluded = excludeGlobs.some(pattern => (0, minimatch_1.minimatch)(file.filename, pattern));
            return !isExcluded;
        });
    }
    /**
     ...
     */
    static chunkDiff(file, maxLines) {
        if (!file.patch) {
            return {
        ...
        if (lines.length <= maxLines) {
            ...
            for (let i = 0; i < lines.length; i++) {
                ...
                const shouldCreateChunk = currentChunk.length >= maxLines || // Reached max lines
     ...
     */
    static parseGlobPatterns(input) {
        if (!input || input.trim() === '') {
            return [];
        }
     ...
     */
    static validateFileChange(file) {
     ...
     */
    static getChangesSummary(files) {
     ...
     */
    static isMeaningfulChunk(chunk) {

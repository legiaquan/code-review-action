    ...
    async review(params) {
        try {
            ...
            if (error && typeof error === 'object' && 'status' in error) {
                ...
                throw new types_1.ProviderError(`OpenAI API error: ${message}`, 'openai', status, error instanceof Error ? error : undefined);
            }
            throw new types_1.ProviderError(`Unexpected error during OpenAI review: ${error instanceof Error ? error.message : 'Unknown error'}`, 'openai', undefined, error instanceof Error ? error : undefined);
        }
    }
    buildPrompt(params) {
